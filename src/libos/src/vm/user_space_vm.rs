...
impl UserSpaceVMManager {
    pub unsafe fn from(addr: usize, size: usize) -> Result<UserSpaceVMManager, Error> {
        let vm_manager = Arc::new(Mutex::new(VMManager::from(addr, size)?));
    ...
    pub fn alloc(&self, size: usize) -> Result<UserSpaceVMRange, Error> {
        let user_vm_range = unsafe {
            ...
            let mut vm_manager = self.vm_manager.lock();
...
impl UserSpaceVMRange {
    fn new(vm_range: VMRange, vm_manager: Arc<Mutex<VMManager>>) -> UserSpaceVMRange {
        UserSpaceVMRange {
            vm_range,
            vm_manager,
        }
